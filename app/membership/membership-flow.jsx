...
export default function MembershipFlow() {
  ...
  const csrLink = (
    <Link href='/csr' className='font-semibold text-blue-700 hover:underline'>
  ...
  const next1 = (e) => {
    ...
    if (emailError) return toast.error("Please enter a valid email address.");
    setStep(2);
  ...
  const submitAndPay = async () => {
    if (form.contribution < 500)
      return toast.error("Minimum support contribution is ₹500");
    setLoading(true);
    try {
  ...
  async function downloadMemberCardPDF() {
    ...
    pdf.save(`Member-Card-${member.memberId}.pdf`);
  }
    ...
    <Card className='border-0 shadow-2xl overflow-hidden'>
      ...
      <div className='bg-gradient-to-r from-slate-50 to-blue-50/50 border-b'>
        <div className='grid grid-cols-4'>
          {["Your details", "Verification", "Contribution", "Welcome"].map(
            (label, i) => {
              const idx = i + 1;
              ...
              return (
                <div
                  key={i}
                  className={`px-3 py-4 text-center text-xs sm:text-sm font-semibold border-r last:border-r-0 ${active ? "bg-white text-blue-900" : done ? "text-emerald-700" : "text-slate-400"}`}
                >
                  <div
                    className={`w-7 h-7 rounded-full mx-auto mb-1.5 flex items-center justify-center text-xs ${active ? "gradient-trust text-white" : done ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-500"}`}
                  >
      ...
      <CardContent className='p-7 lg:p-9'>
        ...
        {step === 1 && (
          <form onSubmit={next1} className='space-y-5'>
            <div>
              <h2 className='font-[Playfair_Display] text-2xl font-bold text-slate-900 mb-1'>
            ...
            <div className='grid sm:grid-cols-2 gap-4'>
              <div className='sm:col-span-2'>
                ...
                <Input
                  ...
                  className='mt-1.5'
                  placeholder='As it should appear on your member card'
                />
              </div>
              <div>
                <Label htmlFor='m-mob'>Mobile number *</Label>
                <Input
                  id='m-mob'
                  required
                  maxLength={10}
              ...
              <div className='sm:col-span-2'>
                <Label htmlFor='m-adr'>Address *</Label>
                <Textarea
                  id='m-adr'
                  required
                  rows={3}
                ...
                <div className='grid grid-cols-3 sm:grid-cols-4 gap-2 mt-1.5'>
                  {OCCUPATIONS.map((o) => (
                    <button
                      ...
                      className={`px-2 py-2 text-xs sm:text-sm rounded-lg border font-medium transition ${form.occupation === o ? "bg-blue-800 text-white border-blue-800" : "bg-white text-slate-700 border-slate-200 hover:border-blue-300"}`}
                    >
                      {o}
                    </button>
                  ))}
            ...
            <div className='pt-2'>
              <Button
                type='submit'
                className='gradient-trust text-white h-11 px-7 font-semibold w-full sm:w-auto'
              >
        ...
        {step === 2 && (
          <form onSubmit={next2} className='space-y-6'>
            <div>
              <h2 className='font-[Playfair_Display] text-2xl font-bold text-slate-900 mb-1'>
                Verification & a few more details
              </h2>
              <p className='text-sm text-slate-500'>
                Identity check helps us protect the integrity of our member
                roll.
              </p>
            </div>
            ...
            <Card className='border-blue-200 bg-blue-50/50'>
              <CardContent className='p-5 flex items-start gap-4'>
                <div className='w-11 h-11 rounded-lg bg-blue-800 flex items-center justify-center shrink-0'>
                ...
                <div className='flex-1'>
                  ...
                  <p className='text-sm text-slate-700'>
                    Identity verification powered by DigiLocker integration is
                    rolling out shortly. Until then, kindly enter your Aadhaar —
                    only the last 4 digits are stored, in masked form.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <div>
              ...
              {form.aadhaar.length === 12 && (
                <p className='text-xs text-emerald-700 mt-1.5 flex items-center gap-1.5'>
                  <Shield className='w-3.5 h-3.5' /> Will be stored as{" "}
                  <span className='font-mono'>{maskAadhaar(form.aadhaar)}</span>
                </p>
              )}
            </div>
              ...
              <div className='mt-1.5 flex items-center gap-4'>
                ...
                <div className='flex-1'>
                  ...
                  <p className='text-xs text-slate-500 mt-2'>
                    JPG/PNG, max 2MB. Will be printed on your member ID card.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                className='mt-1.5'
                placeholder='A few sentences about what brings you to the Trust.'
              />
            </div>
            ...
            <div className='pt-2 flex flex-col-reverse sm:flex-row gap-3 sm:justify-between'>
              ...
              <Button
                type='submit'
                className='gradient-trust text-white h-11 px-7 font-semibold'
              >
                Continue to Contribution <ArrowRight className='w-4 h-4 ml-2' />
              </Button>
            </div>
          </form>
        )}
        ...
        {step === 3 && (
          <div className='space-y-6'>
            <div>
              ...
              <p className='text-sm text-slate-500'>
                A token contribution \u2014 minimum ₹500. 80G tax exempt.
            ...
            <div>
              ...
              <div className='grid grid-cols-5 gap-2 mb-3'>
                {PRESETS.map((a) => (
                  <button
                    type='button'
                    key={a}
                    onClick={() => setForm({ ...form, contribution: a })}
                    className={`py-3 rounded-lg text-sm font-semibold border transition ${form.contribution === a ? "bg-blue-800 text-white border-blue-800" : "border-slate-200 text-slate-700 hover:border-blue-300"}`}
                  >
                    {a >= 1000 ? `${a / 1000}k` : a}
                  </button>
                ))}
              ...
              <Input
                ...
                onChange={(e) => {
                  const contribution = parseInt(e.target.value, 10) || " ";
                  ...
                  if (contribution > MAX_AMOUNT) {
                    setAmountWarning(
                      "Membership contributions are limited to ₹10,000.",
                    );
                  } else if (contribution > 0 && contribution < MIN_AMOUNT) {
                    setAmountWarning(
                      "Minimum membership contribution is ₹500.",
                    );
                  } else {
                    setAmountWarning("");
                  }
                }}
                placeholder='Enter contribution (₹500 - ₹10,000)'
              />
              ...
              {amountWarning && (
                <div className='mt-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800'>
                  ...
                  <p className='mt-2'>
                    If you wish to make a larger contribution, please{" "}
                    {contactUsLink}. Organizations and corporate partners are
                    requested to submit a {csrLink}.
                  </p>
                </div>
              )}
            </div>
            ...
            <Card className='bg-slate-50 border-slate-200'>
              <CardContent className='p-5 space-y-2 text-sm'>
                ...
                <div className='flex justify-between text-lg pt-3 border-t mt-3'>
                  <span className='font-semibold'>Support contribution</span>
                  <span className='font-bold text-blue-900'>
                    ₹{form.contribution.toLocaleString("en-IN")}
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            <div className='px-4 py-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-900 flex items-start gap-2'>
              ...
              <span>
                <strong>Mock mode:</strong> click below to simulate a Razorpay
                payment. Backend will verify the HMAC SHA-256 signature exactly
            ...
            <div className='pt-2 flex flex-col-reverse sm:flex-row gap-3 sm:justify-between'>
              ...
              <Button
                ...
                disabled={loading || form.contribution >= MAX_AMOUNT}
                className={`h-12 px-7 font-bold text-white transition-colors ${
                  loading || form.contribution >= MAX_AMOUNT
              ...
              >
                {loading ? (
                  ...
                  <>
                    Pay ₹{form.contribution.toLocaleString("en-IN")} via
                    Razorpay (mock)
                  </>
                )}
              </Button>
        ...
        {step === 4 && member && (
          <div className='space-y-6'>
            <div className='text-center'>
              ...
              <h2 className='font-[Playfair_Display] text-3xl font-bold text-slate-900 mb-2'>
                Welcome, {member.name.split(" ")[0]} 🙏
              </h2>
              <p className='text-slate-600'>
                Your membership is active. Here is your digital member card
            ...
            <div className='print-receipt'>
              <div
                className='w-[380px] bg-white rounded-2xl overflow-hidden border shadow-2xl mx-auto'
                id='membercard'
              >
                ...
                <div className='px-7 py-6'>
                  <div className='flex gap-4 items-start mb-4'>
                    ...
                    <div className='flex-1 min-w-0'>
                      <div className='font-bold text-slate-900 leading-8 line-clamp-2'>
                        {member.name}
                      </div>
                  ...
                  <div className='grid grid-cols-2 gap-3 text-xs pt-4 border-t'>
                    <div>
                      ...
                      <div className='font-semibold text-slate-900'>
                        {new Date(member.validFrom).toLocaleDateString(
                          "en-IN",
                  ...
                  <div className='mt-4 pt-4 border-t text-[10px] text-slate-500 leading-relaxed'>
                    ...
                    Membership is valid for 12 months from the date of
                    activation and is renewable. Tax exemption under Section 80G
            ...
            <div className='flex flex-col sm:flex-row gap-3 max-w-md mx-auto pt-2 no-print'>
              <Button onClick={downloadMemberCardPDF}>
              ...
              <Button asChild className='flex-1 gradient-trust text-white'>
                <a href='/programs'>
                  <FileCheck2 className='w-4 h-4 mr-2' /> Explore programs
                </a>
              </Button>
            </div>

            <p className='text-center text-xs text-slate-500 max-w-md mx-auto'>
              A welcome email and receipt will reach{" "}
              <strong>{member.email}</strong> shortly. We will also send your
              first quarterly impact report by post.
            </p>
          </div>
        )}
      </CardContent>

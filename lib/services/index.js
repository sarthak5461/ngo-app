...
export async function seedContentBlocks(defaults = {}) {
  const db = await getDb()
  const existing = await db.collection(COLLECTIONS.contentBlocks).find({}, { projection: { key: 1 } }).toArray()
  const existingKeys = new Set(existing.map(r => r.key))
  const toInsert = []
  for (const [key, value] of Object.entries(defaults)) {
    if (existingKeys.has(key)) continue
    toInsert.push({ key, value, updatedBy: 'system_seed', createdAt: new Date(), updatedAt: new Date() })
  }
  if (toInsert.length > 0) {
    await db.collection(COLLECTIONS.contentBlocks).insertMany(toInsert)
  }
...
export async function getContentMap(prefix = '') {
  const db = await getDb()
  const q = prefix ? { key: { $regex: `^${prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}` } } : {}
  const rows = await db.collection(COLLECTIONS.contentBlocks).find(q).toArray()
  const out = {}
...
export async function listMedia({ limit = 100, search = '' } = {}) {
  ...
  const rows = await db.collection(COLLECTIONS.media).find(q).sort({ createdAt: -1 }).limit(limit).toArray()
  return stripIds(rows)
}

export async function addMedia({ id, name, mimeType, size, url, dataUrl, uploadedBy }) {
  const db = await getDb()
  const doc = {
...
export async function getMediaById(id) {
  const db = await getDb()
  const doc = await db.collection(COLLECTIONS.media).findOne({ id })
  if (!doc) return null
  const { _id, ...clean } = doc
  return clean
}

...
export async function listBlogs({ status, limit = 100 } = {}) {
  const database = await getDb();
...
export async function getBlogBySlug(slug) {
  ...
  return database.collection(COLLECTIONS.blogs).findOne({
...
export async function getBlog(id) {
  ...
  const row = await database.collection(COLLECTIONS.blogs).findOne({ id });
...
export async function createBlog(data) {
  ...
  await database.collection(COLLECTIONS.blogs).insertOne(data);
...
export async function updateBlog(id, update) {
  ...
  await database.collection(COLLECTIONS.blogs).updateOne(
    { id },
    {
      $set: update,
    },
...
export async function deleteBlog(id) {
  ...
  await database.collection(COLLECTIONS.blogs).deleteOne({ id });

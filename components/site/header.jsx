...
export default function SiteHeader({ solid = false }) {
  ...
  const brandSubtitle = useContent(
    ...
    "Sangh Trust • since 2008",
  );
  const brandLogo = useContent("header.brand.logo", "");
      ...
      <div className='container flex items-center justify-between py-3'>
        <Link href='/' className='flex items-center gap-2.5'>
          ...
          <div className='hidden sm:block leading-tight'>
            ...
            <div
              className={`text-[11px] tracking-wide ${isLight ? "text-slate-500" : "text-white/80"}`}
            >
              {brandSubtitle}
            </div>
          </div>
        ...
        <nav className='hidden xl:flex items-center gap-7'>
          {NAV.map((it, i) => (
            <Link
              ...
              className={`text-sm font-medium transition-colors ${isLight ? "text-slate-700 hover:text-blue-800" : "text-white/90 hover:text-white"}`}
            >
              {it.label}
            </Link>
          ))}

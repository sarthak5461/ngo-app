    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-lg p-0 overflow-hidden max-h-[92vh] overflow-y-auto'>
        {step === "form" && (
          <>
            <div className='gradient-trust text-white p-6'>
              <DialogHeader>
                <DialogTitle className='text-white text-2xl font-[Playfair_Display]'>
                  Make a Donation
                </DialogTitle>
                <DialogDescription className='text-white/80'>
                  Your contribution is 80G tax-deductible. Receipt issued
                  instantly.
                </DialogDescription>
              </DialogHeader>
            </div>
            <form onSubmit={createOrder} className='p-6 space-y-5'>
                ...
                <div className='grid grid-cols-2 gap-2'>
                  {[
                    ...
                    <button
                      type='button'
                      key={o.k}
                      onClick={() => setCause(o.k)}
                      className={`px-3 py-2.5 rounded-lg text-sm border transition font-medium ${cause === o.k ? "bg-blue-50 border-blue-700 text-blue-900" : "border-slate-200 text-slate-700 hover:border-blue-300"}`}
                    >
                      {o.l}
                    </button>
                  ))}
                ...
                <div className='grid grid-cols-5 gap-2 mb-2'>
                  {PRESET_AMOUNTS.map((a) => (
                    <button
                      ...
                      className={`py-2.5 rounded-lg text-sm font-semibold border transition ${!customAmt && amount === a ? "bg-blue-800 text-white border-blue-800" : "border-slate-200 text-slate-700 hover:border-blue-300"}`}
                    >
                      {a >= 1000 ? `${a / 1000}k` : a}
                    </button>
                  ))}
              ...
              <div className='grid grid-cols-2 gap-3'>
                ...
                <div className='col-span-2'>
                  <Label htmlFor='dpan'>PAN (for 80G receipt &gt;₹2,000)</Label>
                  <Input
                    id='dpan'
                    value={donor.pan}
                    onChange={(e) =>
                      setDonor({ ...donor, pan: e.target.value.toUpperCase() })
                    }
                    className='mt-1.5'
              ...
              <Button
                type='submit'
                disabled={loading}
                className='w-full bg-amber-500 hover:bg-amber-600 text-slate-900 font-bold h-12 text-base'
              >
                {loading ? (
                  <>
                  ...
                  <>
                    Continue to Pay ₹{finalAmount.toLocaleString("en-IN")}{" "}
                    <ArrowRight className='w-4 h-4 ml-2' />
                  </>
                )}
              </Button>
              <p className='text-[11px] text-slate-500 text-center'>
                By donating you agree to our terms. Payments are securely
        ...
        {step === "pay" && order && (
          <div className='p-6'>
            <DialogHeader>
              <DialogTitle className='text-2xl font-[Playfair_Display]'>
                Confirm Payment
              </DialogTitle>
              <DialogDescription>
                Razorpay test/mock checkout — real signature verification on
                backend.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className='flex items-center gap-2 mt-4 mb-5 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-900'>
              ...
              <span>
                <strong>Mock mode:</strong> click below to simulate Razorpay
                success. Backend verifies the HMAC SHA-256 signature exactly
                like production.
              </span>
            </div>
            <Button
            ...
            >
              {loading ? (
                ...
                <>
                  Pay ₹{finalAmount.toLocaleString("en-IN")} via Razorpay (mock)
                </>
              )}
            </Button>
            <Button
        ...
        {step === "receipt" && receipt && (
          <div className='p-0'>
            <div className='print-receipt p-7' id='donation-receipt'>
              <div className='text-center pb-5 border-b'>
                ...
                <h2 className='font-[Playfair_Display] text-2xl font-bold text-slate-900'>
                  Donation Receipt
                </h2>
                <p className='text-lg text-slate-600 mt-1'>
                  श्री जगन्नाथ स्वामी भक्त शिरोमणी माँ कर्मा देवी संघ ट्रस्ट
                </p>
                <p className='text-xs text-slate-500'>
                  12A &amp; 80G Certified • Trust Reg: 432/2024
                </p>
              </div>
              ...
              <div className='py-5 space-y-3 text-sm'>
                ...
                <div className='flex justify-between'>
                  ...
                  <span className='font-medium'>{receipt.donorEmail}</span>
                </div>
                {receipt.panNumber && (
                  <div className='flex justify-between'>
                    <span className='text-slate-500'>PAN</span>
                    <span className='font-mono'>{receipt.panNumber}</span>
                  </div>
                )}
                <div className='flex justify-between'>
                  <span className='text-slate-500'>Cause</span>
                  <span className='font-medium capitalize'>
                    {receipt.cause.replace("-", " ")}
                  </span>
                </div>
                <div className='flex justify-between'>
            ...
            <div className='p-5 bg-slate-50 flex gap-3 no-print'>
              <Button onClick={downloadReceiptPdf}>Download Receipt</Button>
              <Button

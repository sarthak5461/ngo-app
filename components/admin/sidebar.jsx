...
import {
  LayoutDashboard, HeartHandshake, IdCard, Users, Building2,
  FileText, Image as ImageIcon, FileBarChart, Settings, ShieldCheck,
  LogOut, Menu, X, ExternalLink, HandHeart,
} from 'lucide-react'
    ...
    <div className="flex flex-col h-full">
      <div className="px-5 py-5 border-b">
        <Link href="/admin" className="flex items-center gap-2.5">
          <div className="w-10 h-10 rounded-xl gradient-trust flex items-center justify-center"><HandHeart className="w-5 h-5 text-white" /></div>
          <div className="leading-tight">
            <div className="font-bold text-slate-900 text-sm">MKDS Admin</div>
            <div className="text-[11px] text-slate-500">Sangh Trust dashboard</div>
          </div>
        </Link>
      </div>
      ...
      <div className="border-t p-3 space-y-2">
        <Link href="/" target="_blank" className="flex items-center gap-2 px-3 py-2 text-xs text-slate-600 hover:text-blue-800">
        ...
        <div className="flex items-center gap-3 px-3 pt-2 border-t">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[10px] text-slate-500 capitalize">{(user?.role || 'super_admin').replace('_', ' ')}</div>
          </div>
          <button onClick={logout} className="p-2 rounded-md hover:bg-slate-100 text-slate-600" title="Sign out"><LogOut className="w-4 h-4" /></button>
        </div>
      </div>
    </div>
  )
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 inset-x-0 z-40 bg-white border-b flex items-center justify-between px-4 py-3">
        <Link href="/admin" className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-lg gradient-trust flex items-center justify-center"><HandHeart className="w-4 h-4 text-white" /></div>
          <span className="font-bold text-slate-900 text-sm">MKDS Admin</span>
        </Link>
        <button onClick={() => setOpen(true)} className="p-2" aria-label="Open menu"><Menu className="w-5 h-5" /></button>
      </div>
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-50 bg-black/40" onClick={() => setOpen(false)}>
          <div className="absolute inset-y-0 left-0 w-72 bg-white shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-end p-3"><button onClick={() => setOpen(false)} className="p-2"><X className="w-5 h-5" /></button></div>
            <SidebarContent />
          </div>
        </div>
      )}
